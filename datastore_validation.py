...
def perform_validation(tables, data_validator):
    ...
    for key, entry in tables.iteritems():
        ...
        if (entry['resource_retrieval'] == 'fail'):
            continue
...
def main():
    ...
    for type in env['types']:
        ...
        for iteration in range(0,num_of_retrieval):
            ...
            try:
                packages = package_search.packages_with_type(reporter=reporter, type = type,rows= 30, start = start)
            except Exception:
                raise
            # validate each dataset
            for key,value in packages.iteritems():

                tables = datastore_search.tables_in_resources(resources=value['resources'],api_key=env['api_key'],reporter=reporter, package=key)
